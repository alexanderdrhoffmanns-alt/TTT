...
export default function DotsLobby() {
  ...
  useEffect(() => {
    const fetchBotRatings = async () => {
      try {
        const easySnap = await getDoc(doc(db, "users", "bot_dots_easy"));
        const mediumSnap = await getDoc(doc(db, "users", "bot_dots_medium"));
        const hardSnap = await getDoc(doc(db, "users", "bot_dots"));
        
        setBotRatings({
          easy: easySnap.exists() ? (easySnap.data().dots_rating || 800) : 800,
          medium: mediumSnap.exists() ? (mediumSnap.data().dots_rating || 1000) : 1000,
          hard: hardSnap.exists() ? (hardSnap.data().dots_rating || 1200) : 1200
        });
      } catch (e) {
  ...
  useEffect(() => {
    const checkMatchmaking = () => {
      const savedId = sessionStorage.getItem("active_matchmaking_game_dots");
  ...
  const startBotMatch = async (difficulty: "easy" | "medium" | "hard" = "medium") => {
    ...
    if (difficulty === "easy") {
      ...
      botId = "bot_dots_medium";
      botName = "🤖 Medi-Bot (Mittel)";
      startRating = 1000;
    }
    
    try {
      const gameRef = doc(db, "games_dots", savedId);
      ...
      sessionStorage.removeItem("active_matchmaking_game_dots");
      setActiveMatchmakingId(null);
      navigate("/dots/game/" + savedId);
    } catch (error: any) {
  ...
  useEffect(() => {
    const q = query(collection(db, "users"), orderBy("dots_rating", "desc"), limit(50));
    try {
      const unsubscribe = onSnapshot(q, (snapshot) => {
        const users = snapshot.docs
          .map(doc => doc.data() as LeaderboardUser)
          .filter(u => !u.uid.startsWith("bot_") || u.uid.startsWith("bot_dots"));
  ...
  useEffect(() => {
    ...
    const q1 = query(collection(db, "games_dots"), where("player1Id", "==", user.uid));
    const unsubscribe1 = onSnapshot(q1, (snapshot) => {
      ...
      const expectedMatchmakingId = sessionStorage.getItem("active_matchmaking_game_dots");
      if (activeGame && activeGame.id === expectedMatchmakingId) {
        sessionStorage.removeItem("active_matchmaking_game_dots");
        navigate(`/dots/game/${activeGame.id}`);
        return;
      }
      setMyGames(prev => {
    ...
    }, (error) => handleFirestoreError(error, OperationType.LIST, "games_dots"));

    const q2 = query(collection(db, "games_dots"), where("player2Id", "==", user.uid));
  ...
  const findMatch = async () => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        const gameRef = doc(db, "games_dots", gameDoc.id);
        ...
        if (gameDoc.data().player1Id !== user.uid) {
          ...
          navigate(`/dots/game/${gameDoc.id}`);
          return;
        }
      }
      
      const newGameRef = doc(collection(db, "games_dots"));
      ...
      sessionStorage.setItem("active_matchmaking_game_dots", newGameRef.id);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <nav className="shrink-0 h-16 border-b border-slate-200 dark:border-slate-800 px-4 md:px-8 flex items-center justify-between bg-white/80 dark:bg-slate-900/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">DOTS<span className="text-indigo-500">ELITE</span></span>
        </div>
            ...
            <>
              {currentUserData && (
                <div className="hidden md:flex flex-col items-end">
                  ...
                  <span className="text-lg font-mono font-bold text-indigo-400">{currentUserData.dots_rating} ELO</span>
                </div>
              )}
              ...
              {myGames.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {myGames.map(game => {
                    ...
                    const myTurnSymbol = game.player1Id === user.uid ? "blue" : "red";
                    ...
                    return (
                      <div key={game.id} className={cn(
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          {!isWaiting && (
                            <button
                              onClick={() => navigate(`/dots/game/${game.id}`)}
        ...
        <section className="lg:col-span-6 p-6 flex flex-col justify-center items-center relative overflow-y-auto min-h-[400px]">
          ...
          {activeMatchmakingId ? (
            ...
            <div className="flex flex-col items-center text-center gap-8 relative z-10 w-full max-w-lg">
              <div>
                <div className="text-blue-500 text-sm font-semibold mb-1 uppercase tracking-widest">Käsekästchen</div>
                <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white leading-tight">Willkommen im Dots Elite</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium max-w-sm mx-auto">
                  Tritt in der Käsekästchen-Arena an. Verbinde die Punkte, erobere Quadrate und dominiere das Board!
                </p>
              </div>

              <div className="w-56 h-56 opacity-20 border-[4px] border-slate-200 dark:border-slate-800 grid grid-cols-4 grid-rows-4 gap-2 p-4 rounded-2xl pointer-events-none transition-all duration-500 hover:scale-105">
                 {[...Array(16)].map((_, i) => (
                   <div key={i} className="bg-slate-200 dark:bg-slate-800 rounded-lg"></div>
                 ))}
              </div>
              ...
              <button
                ...
                className="group flex items-center justify-center gap-3 bg-blue-600 hover:bg-blue-500 text-white font-extrabold text-lg py-4 px-10 rounded-2xl shadow-xl shadow-blue-900/20 active:scale-95 transition-all duration-200 w-full max-w-xs disabled:opacity-50"
              >
          ...
          <div className="flex-1 overflow-y-auto flex flex-col p-2 gap-1 min-h-[300px]">
              ...
              leaderboard.map((u, i) => {
                ...
                return (
                  <div key={u.uid} className={cn("flex items-center gap-4 p-3 rounded-xl", style.root)}>
                    ...
                    <span className="font-mono font-bold text-sm text-slate-600 dark:text-slate-300 shrink-0">{u.dots_rating}</span>
            ...
            {user && currentUserData && (
              <div className="p-3 mt-4 border-t border-slate-200 dark:border-slate-800 flex items-center gap-4 bg-indigo-600/10 rounded-xl shrink-0">
                ...
                <span className="font-mono font-bold text-sm text-indigo-400">{currentUserData.dots_rating}</span>
              </div>
            )}
          </div>
        </aside>
