...
export default function TicTacToeLobby() {
  ...
  useEffect(() => {
    const fetchBotRatings = async () => {
      try {
        const easySnap = await getDoc(doc(db, "users", "bot_tictactoe_easy"));
        const mediumSnap = await getDoc(doc(db, "users", "bot_tictactoe_medium"));
        const hardSnap = await getDoc(doc(db, "users", "bot_tictactoe"));
        
        setBotRatings({
          easy: easySnap.exists() ? (easySnap.data().rating || 800) : 800,
          medium: mediumSnap.exists() ? (mediumSnap.data().rating || 1000) : 1000,
          hard: hardSnap.exists() ? (hardSnap.data().rating || 1200) : 1200
        });
      } catch (e) {
  ...
  useEffect(() => {
    const checkMatchmaking = () => {
      const savedId = sessionStorage.getItem("active_matchmaking_game_tictactoe");
  ...
  const startBotMatch = async (difficulty: "easy" | "medium" | "hard" = "medium") => {
    ...
    let botId = "bot_tictactoe";
    let botName = "🤖 Robo-Bot (Schwer)";
    let startRating = 1200;
    
    if (difficulty === "easy") {
      botId = "bot_tictactoe_easy";
      botName = "🤖 Easy-Bot (Einfach)";
      startRating = 800;
    } else if (difficulty === "medium") {
      botId = "bot_tictactoe_medium";
      botName = "🤖 Medi-Bot (Mittel)";
      startRating = 1000;
    }
    
    try {
      const gameRef = doc(db, "games", savedId);
      ...
      sessionStorage.removeItem("active_matchmaking_game_tictactoe");
    ...
    try {
      const unsubscribe = onSnapshot(q, (snapshot) => {
        const users = snapshot.docs
          ...
          .filter(u => !u.uid.startsWith("bot_") || u.uid.startsWith("bot_tictactoe"));
  ...
  useEffect(() => {
    ...
    const unsubscribe1 = onSnapshot(q1, (snapshot) => {
      ...
      const expectedMatchmakingId = sessionStorage.getItem("active_matchmaking_game_tictactoe");
  ...
  const findMatch = async () => {
    ...
    try {
      ...
      sessionStorage.setItem("active_matchmaking_game_tictactoe", newGameRef.id);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <nav className="shrink-0 h-16 border-b border-slate-200 dark:border-slate-800 px-4 md:px-8 flex items-center justify-between bg-white/80 dark:bg-slate-900/50 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">TTT<span className="text-indigo-500">ELITE</span></span>
        </div>
            ...
            <>
              {currentUserData && (
                <div className="hidden md:flex flex-col items-end">
                  ...
                  <span className="text-lg font-mono font-bold text-indigo-400">{currentUserData.rating} ELO</span>
                </div>
              )}
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          ...
                          {!isWaiting && (
                            <button
                              ...
                              className="p-2 text-indigo-400 hover:bg-indigo-500/10 rounded-lg transition-colors flex items-center gap-1 text-xs font-bold"
                              title="Beitreten"
                            >
        ...
        <section className="lg:col-span-6 p-6 flex flex-col justify-center items-center relative overflow-y-auto min-h-[400px]">
          ...
          {activeMatchmakingId ? (
            ...
            <div className="flex flex-col items-center text-center gap-8 relative z-10 w-full max-w-lg">
              <div>
                <div className="text-indigo-400 text-sm font-semibold mb-1 uppercase tracking-widest">TicTacToe</div>
                <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 dark:text-white leading-tight">Willkommen im TTT Elite</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium max-w-sm mx-auto">
                  Tritt in der klassischen Tic-Tac-Toe Arena an. Zeige dein Können im ELO-System!
                </p>
              </div>
              ...
              <button
                ...
                className="group flex items-center justify-center gap-3 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-lg py-4 px-10 rounded-2xl shadow-xl shadow-indigo-900/20 active:scale-95 transition-all duration-200 w-full max-w-xs disabled:opacity-50"
              >
          ...
          <div className="flex-1 overflow-y-auto flex flex-col p-2 gap-1 min-h-[300px]">
              ...
              leaderboard.map((u, i) => {
                ...
                return (
                  <div key={u.uid} className={cn("flex items-center gap-4 p-3 rounded-xl", style.root)}>
                    ...
                    <span className="font-mono font-bold text-sm text-slate-600 dark:text-slate-300 shrink-0">{u.rating}</span>
            ...
            {user && currentUserData && (
              <div className="p-3 mt-4 border-t border-slate-200 dark:border-slate-800 flex items-center gap-4 bg-indigo-600/10 rounded-xl shrink-0">
                ...
                <span className="font-mono font-bold text-sm text-indigo-400">{currentUserData.rating}</span>
              </div>
            )}
          </div>
        </aside>
